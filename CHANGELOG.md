...
- 포트폴리오 등록 시 필수 입력 항목 강화 — 시공 위치, 공간 유형, 시공 범위, 시작일, 완료일이 이제 API와 프론트엔드 양쪽에서 필수로 강제됨
- 포트폴리오 wizard에서 필수 단계(Step 1, Step 4) Skip 버튼 제거 — 선택 단계(Step 3 상세 정보)에만 건너뛰기 허용
- 포트폴리오 submit 전 모든 필수 필드 누락 여부 사전 검증
